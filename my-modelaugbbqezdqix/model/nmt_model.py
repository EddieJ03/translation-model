...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(embed_size, self.hidden_size, bias=False, bidirectional=True)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        split = torch.split(Y, split_size_or_sections=1)
        
        for Y_t in split:
            Y_t = torch.squeeze(Y_t)
            Ybar_t = torch.cat((Y_t, o_prev), dim=-1)
            
            dec_state, combined_output, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, None)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = enc_hiddens_proj @ torch.unsqueeze(dec_hidden, dim=-1)
        ...
        # 2
        a_t = torch.unsqueeze(alpha_t, dim=-2) @ enc_hiddens
        
        # 3
        U_t = torch.cat((torch.squeeze(a_t, dim=-2), dec_hidden), dim=-1)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int = 5, max_decoding_time_step: int = 70) -> List[
        ...
        assert len(completed_hypotheses) + len(hypotheses) == beam_size

        if len(completed_hypotheses) < beam_size:
            items = list(zip(hypotheses, hyp_scores))
            
            items.sort(key=lambda i: i[1], reverse=True)
                
            for i in range(min(beam_size - len(completed_hypotheses), len(items))):
                completed_hypotheses.append(Hypothesis(value=''.join(items[i][0][1:]).replace('▁', ' ').strip(),
                                                    score=items[i][1].item()))
